...
**Theorem 3.2** We denote the approximation error in GOSS as $\varepsilon(d),\bar{g}^j_l(d)=\frac{\sum_{x_i\in(A\cup A^c)_l}\lvert g_i\rvert}{n^j_l(d)}\text{ and }\bar{g}^j_r(d)=\frac{\sum_{x_i\in(A\cup A^c)_r}\lvert g_i\rvert}{n^j_r(d)}$ as the average value of absolute gradient values in, $l$, left split and $r$, right split, respectively. With probability at least $1-\delta$, we have
